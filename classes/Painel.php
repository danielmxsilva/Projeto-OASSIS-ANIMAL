<?php
	...
	class Painel
	{
		...
		public static function generateSlug($str){
			...
			$str = preg_replace('/(í|Í|Ì|ì)/', 'i', $str);
		...
		public static function limparApadrinhado(){
			$date = date('Y-m-d');
			$sql = Mysql::conectar()->prepare("UPDATE `tb_site.apadrinhar` SET categoria_id = ? WHERE data < '$date' - INTERVAL 15 DAY");
			$sql->execute(array(2));
		}
		...
		public static function updateConfig($arr){
			...
			foreach($arr as $key => $value){
				$nome = $key;
				$valor = $value;
				if($nome == 'acao_editar' || $nome == 'nome_tabela' || $nome == 'id' || $nome == 'imagem_autor_atual' || $nome == 'imagem_especial_atual1' || $nome == 'imagem_especial_atual2' || $nome == 'imagem_especial_atual3')
					continue;
				if($value == ' '){
					$certo = false;
					break;
