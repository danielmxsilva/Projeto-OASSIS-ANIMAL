		...
		if(isset($_POST['acao_adicionar'])){
			...
			if($nome == '' || $descricao == ''){
				...
				if(Painel::imagemValida($foto)){
					$verifica = Mysql::conectar()->prepare("SELECT * FROM `tb_site.apadrinhar` WHERE nome = ?");
					$verifica->execute(array($nome));
					if($verifica->rowCount() == 0){
						$foto = Painel::uploadFile($foto);
						$slug = Painel::generateSlug($nome);
						$arr = ['nome'=>$nome,'foto'=>$foto,'slug'=>$slug,'descricao'=>$descricao,'categoria_id'=>'2','padrinho'=>'','email'=>'','valor'=>'','compra'=>'','order_id'=>'0','nome_tabela'=>'tb_site.apadrinhar','data'=>''];
						...
						Painel::alert('erro','Já um animal com esse nome!');
					}
				}else{
	...
	<div class="form-editar cadastro-depoimentos">
		<form method="POST" enctype="multipart/form-data">
			...
			<div class="group-depoimento">
				...
				<input type="hidden" name="nome_tabela" value="tb_site.apadrinhar">
