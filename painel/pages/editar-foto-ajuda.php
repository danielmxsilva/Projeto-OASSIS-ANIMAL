<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$slide_editar = Painel::select('tb_foto.ajuda','id = ?',array($id));
	}else{
		...
		if(isset($_POST['acao_editar'])){
			...
			if($imagem['name'] != ''){
				if(Painel::imagemValida($imagem)){
					...
					$arr = ['nome'=>$nome,'foto'=>$imagem,'nome_tabela'=>'tb_foto.ajuda','id'=>$id];
					Painel::update($arr);
					Painel::alert('sucesso','Foto Editada Com Sucesso!');
					$slide_editar = Painel::select('tb_foto.ajuda','id = ?',array($id));
					Painel::redirect(INCLUDE_PATH_PAINEL.'editar-foto-ajuda?id='.$id);
				}else{
