...
<div class="box-content">
	...
	<?php
		if(isset($_POST['acao_editar'])){
			...
			$verificar = Mysql::conectar()->prepare("SELECT `id` FROM `tb_site.apadrinhar` WHERE nome = ? AND id != ?");
			...
			if($imagem['name'] != ''){
				if(Painel::imagemValida($imagem)){
					...
					$arr = ['nome'=>$nome,'foto'=>$imagem,'nome_tabela'=>'tb_site.apadrinhar','slug'=>$slug,'descricao'=>$descricao,'padrinho'=>'','email'=>'','valor'=>'','compra'=>'','data'=>'','id'=>$id,'order_id'=>$id];
					Painel::update($arr);
					Painel::alert('sucesso','Animal Editado Com Sucesso!');
					$noticia_editar = Painel::select('tb_site.apadrinhar','id = ?',array($id));
					Painel::redirect(INCLUDE_PATH_PAINEL.'editar-sem-padrinho?id='.$id);
				}else{
