<?php
	if(isset($_GET['excluir'])){
		...
		$selectImagem = Mysql::conectar()->prepare("SELECT foto FROM `tb_foto.mural` WHERE id = ?");
		$selectImagem->execute(array($_GET['excluir']));
		$imagem = $selectImagem->fetch()['foto'];
		Painel::deleteFile($imagem);
		Painel::deletar('tb_foto.mural',$idExcluir);
		Painel::redirect(INCLUDE_PATH_PAINEL.'gerenciar-mural');
	}else if(isset($_GET['order']) && isset($_GET['id'])){
		Painel::orderItem('tb_foto.mural',$_GET['order'],$_GET['id']);
	}
?>
