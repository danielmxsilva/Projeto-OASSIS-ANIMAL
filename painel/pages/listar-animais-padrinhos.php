	...
	<div class="table-wraper">
		...
		?>
			<tbody>
				...
				<td class="tb-editar">
						<a href="<?php echo INCLUDE_PATH_PAINEL?>editar-animal-padrinho?id=<?php echo $value['id']?>"><img src="img/editar-depoimento-verde.png"></a>
				</td>
				<td class="tb-excluir">
						<a <?php
							if($_SESSION['cargo'] >= 1){
	...
	<div class="paginacao">
		<?php
			...
			for($i = 1; $i <= $totalPagina; $i++){
				if($i == $paginaAtual){
					echo '<a class="pag-selected" href="'.INCLUDE_PATH_PAINEL.'gerenciar-padrinhos?pagina='.$i.'">'.$i.'</a>';
				}else{
					echo '<a href="'.INCLUDE_PATH_PAINEL.'gerenciar-padrinhos?pagina='.$i.'">'.$i.'</a>';
				}
			}
		?>
